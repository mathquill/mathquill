...
function createRoot(jQ, root, textbox, editable) {
  ...
  jQ.bind('mousedown.mathquill', function(e) {
    function mousemove(e) {
      ...
      if (cursor.left !== anticursor.left
    ...
    anticursor = {parent: cursor.parent, left: cursor.left, right: cursor.right};
...
var RootMathBlock = P(MathBlock, function(_, _super) {
  ...
  _.renderLatex = function(latex) {
    ...
    this.leftmostChild = this.rightmostChild = 0;
  ...
  _.onKey = function(key, e) {
    ...
    case 'Ctrl-Backspace':
      while (this.cursor.left || this.cursor.selection) {
    ...
    case 'Tab':
      ...
      if (parent.right) {
        // go one block right
        this.cursor.prependTo(parent.right);
      } else {
    ...
    case 'Shift-Tab':
      ...
      if (parent.left) {
        // go one block left
        this.cursor.appendTo(parent.left);
      } else {
        //get out of the block
    ...
    case 'Ctrl-Shift-End':
      while (this.cursor.right || this.cursor.parent !== this) {
        this.cursor.selectRight();
      }
      break;
    ...
    case 'Ctrl-Shift-Home':
      while (this.cursor.left || this.cursor.parent !== this) {
        this.cursor.selectLeft();
      }
      break;
    ...
    case 'Shift-Up':
      if (this.cursor.left) {
        while (this.cursor.left) this.cursor.selectLeft();
      } else {
        this.cursor.selectLeft();
      }
    ...
    case 'Shift-Down':
      if (this.cursor.right) {
        while (this.cursor.right) this.cursor.selectRight();
      }
      else {
        this.cursor.selectRight();
      }
    ...
    case 'Del':
      if (e.ctrlKey)
        while (this.cursor.right || this.cursor.selection)
...
var RootMathCommand = P(MathCommand, function(_, _super) {
  ...
  _.createBlocks = function() {
    ...
    this.blocks = [ this.leftmostChild ];

    this.leftmostChild.parent = this;

    var cursor = this.leftmostChild.cursor = this.cursor;
    this.leftmostChild.onText = function(ch) {
  ...
  _.latex = function() {
    return '$' + this.leftmostChild.latex() + '$';
  };
});
...
var RootTextBlock = P(MathBlock, function(_) {
  _.renderLatex = function(latex) {
    ...
    self.leftmostChild = self.rightmostChild = 0;
    ...
    var mathMode = string('$').then(latexMathParser)
      ...
      .map(function(block) {
        ...
        var rootMathBlock = rootMathCommand.leftmostChild;
    ...
    if (commands) {
      for (var i = 0; i < commands.length; i += 1) {
        commands[i].adopt(self, self.rightmostChild, 0);
      }
